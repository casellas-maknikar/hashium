class HybridRouter {
  ...
  /* ---------- mutation observer settle ---------- */

  waitForSection(section, callback) {
    const expectedId = section
      ? `${this.unpath(section)}-section`
      : 'home-section';
  ...
  driveFromClick(section) {
    ...
    // creates ONE history entry (hash)
    this.l.hash = targetHash;

    // wait until Carrd activates the section
    this.waitForSection(section, () => {
      this.cleanTo(section);   // normalize same entry
      this._driving = false;
    });
  }
  ...
  init() {
    ...
    // External hash changes (keyboard, scripts)
    this.aEL('hashchange', () => {
