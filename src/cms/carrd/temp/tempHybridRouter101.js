...
class HybridRouter {
  ...
  init() {
    ...
    t.aEL('hashchange', () => {
      ...
      // If there is no hash (empty string), do nothing. This prevents
      // accidentally re-cleaning after we remove a section hash via
      // replaceState, which would otherwise strip the section path.
    ...
    t.aEL('popstate', (e) => {
      ...
      /*
       * For scrollpoint restores we need to ensure that the underlying
       * Carrd site switches to the correct section before we attempt to
       * scroll to the scrollpoint. Previously we skipped calling
       * drive() entirely which left the wrong section active (e.g. the
       * home section would remain active when navigating back to
       * `/page#test`). To fix this we manually update the location.hash
       * to the section anchor via `replace()` while suppressing our own
       * hashchange handler with `_driving`. After the section is
       * activated we restore the scrollpoint and update the state.
       */
      const sectionHash = t.hashFor(section);
      // If the current hash does not match the base section hash,
      // update it using location.replace(). This triggers Carrd to
      // activate the correct section without pushing a new entry.
      ...
      setTimeout(() => {
        ...
        // Reset driving flag once we've finished restoring
        t._driving = 0;
      }, ms + 30);
    });
  }
