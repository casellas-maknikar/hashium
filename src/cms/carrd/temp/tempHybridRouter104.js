class HybridRouter {
  ...
  init() {
    ...
    // Hashchange: ignore scrollpoints (we own them) + de-dupe section cleanup
    t.aEL('hashchange', () => {
      ...
      // Section hash cleanup with de-dupe
      const targetSection = t.sectionFromHash(l.hash);
      const now = Date.now();

      // If we just cleaned this exact section hash moments ago, ignore repeat.
      ...
      // Normalize to clean URL (REPLACE only)
      setTimeout(() => {
        t.rS({ section: targetSection }, '', `${t.o}/${targetSection || ''}`);
      }, t.SETTLE_MS);
    });
    ...
    t.aEL('popstate', (e) => {
      ...
      // Fragment restore: activate correct section first (Carrd), then scroll.
